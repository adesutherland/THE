...
static void print_capabilities(void)
{
   ...
   fputs(",\"limitations\":[\"not wired to the full THE command dispatcher\",\"only SOS navigation/edit subset commands are supported by the agent subset\",\"SOS PREFIX focuses the prefix field but full prefix commands remain unsupported\",\"CREXX macros require the full editor integration surface\",\"popup/dialog behavior is not modeled yet\",\"mouse support accepts logical hit targets, not terminal mouse packets\"]", stdout);
