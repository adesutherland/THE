...
calculation, movement, software cursor cell painting, render-entry cursor
save/restore, cursor repaint transitions, and refresh. `src/show.c` keeps its
...
5. Consolidate software cursor painting into one driver-owned path: partial.
   ...
   and prefix overlay ownership; software-cursor attribute, cell painting, and
   render-entry cursor save/restore helpers now live in `cursesdriver.c`.
   Targeted redraws still need to become driver-level logical render requests
   instead of relying on legacy snapshots.
