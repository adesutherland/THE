...
| Protocol verbs | Implemented | The real-runtime protocol accepts `look`, `delta`, `capabilities`, `focus`, `hit`, `key`, `text`, `type`, `text-utf`, `insert`, `insert-utf`, `command`, `debug`, `transient`, and `quit`. | `test_the_llm_full_runtime`, `test_inputevent` |
...
| Cursor movement/reporting | Implemented | Snapshots report logical zone, file line, row, cell, desired cell, and focused row. Protocol `focus`, `hit`, `key`, `text`, `type`, `text-utf`, explicit `insert after LINE TEXT`, and `insert-utf after LINE ...` enter through the real runtime. | `test_the_llm_full_runtime`, `test_inputevent`, `test_llmdriver` |
| Prefix command model | Implemented through real runtime | Prefix state and execution are THE's real prefix model. Snapshots expose prefix text and semantic prefix command fields where the runtime has them, and ACKs report queued pending prefix commands. | `test_the_llm_full_runtime`, `test_llmdriver` |
