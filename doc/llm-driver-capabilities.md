...
| Protocol verbs | Implemented | The real-runtime protocol accepts `look`, `delta`, `capabilities`, `focus`, `hit`, `key`, `text`, `type`, `insert`, `command`, `debug`, `transient`, and `quit`. | `test_the_llm_full_runtime`, `test_inputevent` |
...
| Cursor movement/reporting | Implemented | Snapshots report logical zone, file line, row, cell, desired cell, and focused row. Protocol `focus`, `hit`, `key`, `text`, `type`, and explicit `insert after LINE TEXT` enter through the real runtime. | `test_the_llm_full_runtime`, `test_inputevent`, `test_llmdriver` |
