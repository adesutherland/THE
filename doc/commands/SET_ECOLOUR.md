...
## Syntax
```text
[SET] ECOLOUR element|char [modifier[...]] [foreground] [on background]
[SET] ECOLOUR element|char [modifier[...]] ON|OFF
```

## Description
The `SET ECOLOUR` (or `SET ECOLOR`) command allows the user to specify the colours of each category of syntax elements used in syntax highlighting.
