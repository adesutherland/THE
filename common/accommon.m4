...
echo $with_cursesincdir
   if test "$with_cursesincdir" = no; then
      ...
      MH_CURSES_INC="$with_cursesincdir"
      MH_CURSES_LIB="-L$with_curseslibdir -lpdcurses -lSDL2 -lSDL2_ttf"
   fi
fi
])
...
case "$with_curses" in
   ncurses|ncursesw|ncursesw8)
      ...
      ;;
   pdcurses-x11)
      MH_CHECK_PDCURSES_X11($with_curses,$1,'','',$X11_LIBS)
      ;;
   pdcurses-x11w)
      MH_CHECK_PDCURSES_X11($with_curses,$1,'','-DPDC_WIDE',$X11_LIBS)
      ;;
   pdcurses-x11w8)
      MH_CHECK_PDCURSES_X11($with_curses,$1,'','-DPDC_WIDE -DPDC_FORCE_UTF8',$X11_LIBS)
      ;;
   pdcurses-sdl1)
      MH_CHECK_PDCURSES_SDL($with_curses,$1,sdl,'','-lSDL')
      ;;
   pdcurses-sdl1w)
      MH_CHECK_PDCURSES_SDL($with_curses,$1,SDL_ttf,'-DPDC_WIDE','-lSDL -lSDL_ttf')
      ;;
   pdcurses-sdl1w8)
      MH_CHECK_PDCURSES_SDL($with_curses,$1,SDL_ttf,'-DPDC_WIDE -DPDC_FORCE_UTF8','-lSDL -lSDL_ttf')
      ;;
   pdcurses-sdl2)
      MH_CHECK_PDCURSES_SDL($with_curses,$1,sdl2,'','-SDL2')
      ;;
   pdcurses-sdl2w)
      MH_CHECK_PDCURSES_SDL($with_curses,$1,SDL2_ttf,'-DPDC_WIDE','-lSDL2 -lSDL2_ttf')
      if test "$enable_font_check" = yes; then
         MH_CHECK_PDCURSES_FONT
      fi
      ;;
   pdcurses-sdl2w8)
      MH_CHECK_PDCURSES_SDL($with_curses,$1,SDL2_ttf,'-DPDC_WIDE -DPDC_FORCE_UTF8','-lSDL2 -lSDL2_ttf')
      if test "$enable_font_check" = yes; then
         MH_CHECK_PDCURSES_FONT
      fi
      ;;
