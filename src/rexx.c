...
#if !defined(USE_XCURSES) && !defined(USE_SDLCURSES) && !defined(USE_WINGUICURSES)
         else
         {
            /*
...
/*--------------------------- local data ------------------------------*/
 int rc=0;
 static CHARTYPE num0[15];                  /* large enough for 1000+rc */
...
/***********************************************************************/
{
    ...
    */
#if defined(USE_WINGUICURSES) || (defined(USE_SDLCURSES) && defined(_WIN32))
   /* do our redirection on the command line; very clunky! */
   {
      int len = 100;
      char *buf;
      if (out) len += strlen(outfile);
      if (err) len += strlen(errfile);
      if (in) len += strlen(infile);
      buf = alloca( len );
      buf[0] = '\0';
      strcat( buf, cmd );
      strcat( buf, " " );
   ...
   {
      rc = set_rexx_variable( outstem, (CHARTYPE *)"0", 1, 0 );
   }
   if (err)
   {
      if (!out_and_err_same)
   ...
   {
      rc = set_rexx_variable( errstem, (CHARTYPE *)"0", 1, 0 );
   }
   /*
