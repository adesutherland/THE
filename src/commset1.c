...
/***********************************************************************/
{
   ...
   switch (valid_areas[area].area_window)
   {
      case WINDOW_FILEAREA:
         ...
         if ( area == ATTR_BOUNDMARK)
         {
            the_driver->redraw_window(driver_current_role_window(WINDOW_FILEAREA));
            driver_touch_and_refresh_window(driver_current_role_window(WINDOW_FILEAREA));
         }
#endif
         break;
      case WINDOW_PREFIX:
      ...
      case WINDOW_COMMAND:
         if (driver_current_role_exists(WINDOW_COMMAND))
         {
            ...
            the_driver->redraw_window(driver_current_role_window(WINDOW_COMMAND));
      ...
      case WINDOW_ARROW:
         if (driver_current_role_exists(WINDOW_ARROW))
         {
            ...
            driver_touch_and_refresh_window(driver_current_role_window(WINDOW_ARROW));
         }
         break;
      case WINDOW_IDLINE:
         if (driver_current_role_exists(WINDOW_IDLINE))
         {
            ...
            driver_touch_and_refresh_window(driver_current_role_window(WINDOW_IDLINE));
         }
         break;
      case WINDOW_STATAREA:
         if (driver_global_window_exists(THE_DRIVER_GLOBAL_STATAREA))
         {
            ...
            driver_touch_and_refresh_window(driver_global_window(THE_DRIVER_GLOBAL_STATAREA));
         }
         break;
      case WINDOW_FILETABS:
         if (driver_global_window_exists(THE_DRIVER_GLOBAL_FILETABS))
         {
            ...
            the_driver->redraw_window(driver_global_window(THE_DRIVER_GLOBAL_FILETABS));
            driver_touch_and_refresh_window(driver_global_window(THE_DRIVER_GLOBAL_FILETABS));
         }
         break;
      case WINDOW_DIVIDER:
         if (driver_global_window_exists(THE_DRIVER_GLOBAL_DIVIDER))
         {
...
DESCRIPTION
     ...
     'TAB' will set the up arrow to move to the last line of the main window.
     The down arrow and shift-down arrow will retrieve the older and newer
     command respectively from the <command line>. In addition, when at the
     bottom of the file area, the down arrow jumps to the <command line>.
...
#define COM_PARAMS  4
    ...
    */
   if (curses_started
   &&  driver_global_window_exists(THE_DRIVER_GLOBAL_STATAREA))
   {
...
short get_ecolour_area(CHARTYPE *word)
{
    struct {
        ...
        {"paren", ECOLOUR_PAREN},
        {"type", ECOLOUR_TYPES},
        {"constant", ECOLOUR_CONSTANTS},
        {"macro_name", ECOLOUR_MACROS},
        {"macro_identifier", ECOLOUR_MACROS},
        {"macro_variable", ECOLOUR_MACRO_VARIABLES},
        {"macro_var", ECOLOUR_MACRO_VARIABLES},
        {"macro_constant", ECOLOUR_MACRO_CONSTANTS},
        {"punctuation", ECOLOUR_PUNCTUATION},
        {"match", ECOLOUR_MATCH},
        {"operator", ECOLOUR_OPERATOR},
...
DESCRIPTION
     The SET FDISPLAY command allows the user to change the display filename of
     the file currently being edited as shown on the <IDLINE>.
...
STATUS
     Complete.
**man-end**********************************************************************/
short Fdisplay(CHARTYPE *params)
/***********************************************************************/
{
   short rc=RC_OK;

   TRACE_FUNCTION("commset1.c:Fdisplay");
  /*
   * Must supply a parameter...
   */
   if ( blank_field( params ) )
    ...
    */
   if ( strlen( (DEFCHAR*)params ) > strlen( (DEFCHAR*)CURRENT_FILE->display_name ) )
   {
      (*the_free)( CURRENT_FILE->display_name );
      if ( ( CURRENT_FILE->display_name = (CHARTYPE *)(*the_malloc)( strlen( (DEFCHAR *)params ) + 1 ) ) == NULL )
      {
         display_error( 30, (CHARTYPE *)"", FALSE );
         TRACE_RETURN();
         return(RC_OUT_OF_MEMORY);
   ...
   strcpy( (DEFCHAR *)CURRENT_FILE->display_name, (DEFCHAR *)params );
   /*
...
/***********************************************************************/
{
   ...
   {
      if ( FILETABSx && !driver_global_window_exists(THE_DRIVER_GLOBAL_FILETABS) )
      {
         create_filetabs_window();
      }
      else if ( !FILETABSx && driver_global_window_exists(THE_DRIVER_GLOBAL_FILETABS) )
      {
...
DESCRIPTION
     The SET MOUSECLICK command specifies the time interval in milliseconds
     that THE uses to determine if a mouse button CLICK is returned or a pair
     of mouse button PRESS and RELEASE events are returned.
     See APPENDIX 3 for details on default mouse support.
...
NOTES
     Setting this value to 0 results in only mouse button PRESS/RELEASE events
     being returned.

STATUS
     Complete.
**man-end**********************************************************************/
short Mouseclick(CHARTYPE *params)
/***********************************************************************/
{
   short rc=RC_OK;
   int interval;
#define MC_PARAMS  1
   ...
   TRACE_FUNCTION("commset1.c:Mouseclick");
   /*
    * Two parameters are mandatory, the third is optional.
    */
   strip[0]=STRIP_BOTH;
   ...
   if ( !valid_positive_integer( word[0] ) )
   {
      display_error(4,(CHARTYPE *)"",FALSE);
      TRACE_RETURN();
      return(RC_INVALID_OPERAND);
   }
