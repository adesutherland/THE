...
/*                dst_view->focus_line = dst_view->current_line; */
   ...
   &&  dst_view == CURRENT_SCREEN.screen_view)
   {
      if (dst_view->current_window == WINDOW_PREFIX)
         ...
         the_driver->move_filearea_cursor(current_screen, dst_view,
...
/***********************************************************************/
{
   ...
   if (logical_col < 0)
      logical_col = 0;
   the_driver->move_filearea_cursor(curr_screen, curr_view, line, len,
                                      row, logical_col);
}
/***********************************************************************/
...
/*            p = make_upper( p ); not needed as set_editv() uppercases this anyway */
      ...
      case EDITV_GET:
         ...
         break;
      case EDITV_GETSTEM:
         len_name = strlen( (DEFCHAR *)params );
         ...
         {
            if (curr->name
            &&  strlen((DEFCHAR *)curr->name) > len_name
            &&  memcmpi(curr->name, params, len_name) == 0)
            {
               if ( curr->line )
                  str = curr->line;
               else
                  str = (CHARTYPE *)"";
               rc = set_rexx_variable( curr->name, str, strlen( (DEFCHAR*)str ), -1 );
               if ( rc != RC_OK )
                  break;
            }
            curr = curr->next;
...
#define STATE_POPUP_KEYS             9
   ...
   TheDriverScreenPoint screen_point;
   CHARTYPE **args=NULL;
