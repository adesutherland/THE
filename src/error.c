...
static CHARTYPE _THE_FAR *error_message[] =
{
   ...
   (CHARTYPE *)"Error 0149: Operand in stem operation requires trailing period:",
   (CHARTYPE *)"",
   (CHARTYPE *)"",
   (CHARTYPE *)"",
   (CHARTYPE *)"",
...
/***********************************************************************/
{
   ...
   if ( error_window != NULL )
      the_driver->delete_window(error_window);
   error_window = the_driver->create_window(
...
/***********************************************************************/
{
                            ...
                            ||  CLEARERRORKEYx == -1 ) )
   {
      ...
      if (CURRENT_VIEW != NULL && driver_current_window_exists())
      {
         int y=0, x=0;
         TheDriverWindowCursor cursor;
...
/***********************************************************************/
{
   ...
   the_driver->move_window_cursor(driver_global_window(THE_DRIVER_GLOBAL_ERROR), 0, 0);
...
#define IN_MACRO_PROMPT "Press SPACE to terminate macro or any other key to continue..."
   ...
   {
      the_driver->move_window_cursor(driver_global_window(THE_DRIVER_GLOBAL_ERROR), i, 0);
      my_wclrtoeol( error_window );
      if ( CURRENT_VIEW == NULL
      ||   CURRENT_FILE == NULL)
         the_driver->add_global_string_at(THE_DRIVER_GLOBAL_ERROR, i, 0, (DEFCHAR *)curr_error->line);
      else
         put_string( error_window, (ROWTYPE)i, 0, curr_error->line, curr_error->length );
      curr_error = curr_error->prev;
   }
   the_driver->refresh_window(driver_global_window(THE_DRIVER_GLOBAL_ERROR));
   error_on_screen = TRUE;
   ...
   &&   curr_error != NULL )
   {
      ...
      the_driver->move_window_cursor(driver_global_window(THE_DRIVER_GLOBAL_ERROR), msgline_rows - 1, 0);
      my_wclrtoeol( error_window );
      if ( CURRENT_VIEW == NULL
      ||   CURRENT_FILE == NULL)
      {
         the_driver->add_global_string_at(THE_DRIVER_GLOBAL_ERROR, msgline_rows - 1, 0, (DEFCHAR *)prompt);
      }
      else
      {
