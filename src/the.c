...
/***********************************************************************/
{
   ...
   */
   ...
   term_name = (CHARTYPE *)"SDL";
#elif defined(USE_VTCURSES)
   term_name = (CHARTYPE *)"VT";
#elif defined(USE_WINGUICURSES)
   term_name = (CHARTYPE *)"WINGUI";
   ...
   while ((c = my_getopt( my_argc, my_argv, mygetopt_opts ) ) != EOF )
   {
      switch((char)c)
      {
         ...
         case 'k':        /* allow Soft Label Keys */
            ...
            {
               cleanup();
               DISPLAY_ERROR(4,(CHARTYPE *)optarg,FALSE,1);
               return(4);
            }
            if ( slk_format > MAX_SLK_FORMAT )
            {
               ...
               DISPLAY_ERROR(6,(CHARTYPE *)xterm_program,FALSE,1);
               return(4);
            }
         ...
         case 'l':        /* set current line on startup */
            ...
            {
               cleanup();
               DISPLAY_ERROR(5,(CHARTYPE *)"startup line MUST be > 0",FALSE,1);
               return(4);
            }
            break;
         case 'c':        /* set current column on startup */
            ...
            {
               cleanup();
               DISPLAY_ERROR(5,(CHARTYPE *)"startup column MUST be > 0",FALSE,1);
               return(4);
            }
            break;
         ...
         case 'w':        /* width of line */
            if ( ( rc = valid_positive_integer_against_maximum( (CHARTYPE *)optarg, MAX_WIDTH_NUM ) ) != 0 )
            {
               ...
               DISPLAY_ERROR( rc, (CHARTYPE *)mygetopt_opts, FALSE,1 );
               return(4);
            }
            ...
            {
               cleanup();
               DISPLAY_ERROR(5,(CHARTYPE *)"- width MUST be >= 10",FALSE,1);
               return(4);
            }
            if (max_line_length > MAX_WIDTH_NUM)
            {
               ...
               /* safe to use mygetopt_opts as we are bailing out */
               sprintf( mygetopt_opts, "- width MUST be <= %ld", MAX_WIDTH_NUM );
               DISPLAY_ERROR( 6, (CHARTYPE *)mygetopt_opts, FALSE, 1 );
               return(5);
            }
            break;
         case 'u':        /* display length */
            ...
            {
               cleanup();
               DISPLAY_ERROR(5,(CHARTYPE *)"- display length MUST be > 0",FALSE,1);
               return(4);
            }
            break;
#if defined(USE_XCURSES) && PDC_BUILD >= 2401
...
# endif
         ...
         default:
            cleanup();
            DISPLAY_ERROR(0,(CHARTYPE *)"Invalid command line switch",FALSE,1);
            return(0);
            break;
      }
   }
...
# endif
   ...
   {
      cleanup();
      DISPLAY_ERROR(6,(CHARTYPE *)"- width MUST be >= display length",FALSE,1);
      return(8);
   }
   /*
   ...
   &&  batch_only)
   {
      FILE *fp=NULL;
      int num=0;
      if ( ( stdinprofile = (CHARTYPE *)thetmpnam ( "PRF" ) ) == NULL )
      {
         cleanup();
         return(31);
      }
   ...
   {
      cleanup();
      DISPLAY_ERROR( 7,dir_pathname, FALSE, 1 );
      return(18);
   }
    ...
    */
   if ( ( tempfilename = (CHARTYPE *)thetmpnam ( "TMP" ) ) == NULL )
   {
      cleanup();
      return(31);
   }
   ...
   if (batch_only)
   {
      ...
      while(current_file_name != NULL)
      {
        if ((rc = get_file((CHARTYPE *)current_file_name->line)) != RC_OK)
        {
           ...
           if (rc == RC_DISK_FULL)
           {
              DISPLAY_ERROR(57,(CHARTYPE *)"...probably",FALSE,1);
           }
           return(21);
        }
...
# endif
   ...
   {
      cleanup();
      DISPLAY_ERROR(0,(CHARTYPE *)"creating status line window",FALSE,1);
      return(23);
   }
...
/***********************************************************************/
{
   ...
   if (specified_prf != (CHARTYPE *)NULL)
   {
      ...
      if ( rc != RC_OK)
      {
         DISPLAY_ERROR(9,specified_prf,FALSE,1);
         (*the_free)(local_prf);
         local_prf = NULL;
         return(rc);
      }
      ...
      {
         DISPLAY_ERROR(9,local_prf,FALSE,1);
         (*the_free)(local_prf);
         local_prf = NULL;
      ...
      {
         DISPLAY_ERROR(8,local_prf,FALSE,1);
         (*the_free)(local_prf);
         local_prf = NULL;
...
void ClosedownConsole( int pause )
{
   if ( gotOutput )
   {
      if ( pause )
