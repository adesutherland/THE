...
extern ExtractFunction extract_stylespans;
...
ERRORFormat
     Indicates if THE errors are displayed with extended information.
...
FDISPLAY
     The optional file display name of the current file.
     (QEM)

     fdisplay.0      - 1
     fdisplay.1      - File display name.
...
MOUSEClick
     Returns the time in milliseconds that THE uses to distinguish between
     a mouse CLICK and a pair of PRESS/RELEASE events.
     Set by <SET MOUSECLICK>.
     (QEM)

     mouseclick.0    - 1
     mouseclick.1    - a positive number, 0 or -1 if no mouse support
...
SELect
     ...
     select.3        - maximum valid selection level
...
STYLESPANS
     ...
     Lines are one-based file lines. Cells are zero-based logical cells.
     Style names use the same lowercase logical categories as the LLM driver.
     (E)
...
VARiant
     ...
     variant.0       - 1
     variant.1       - terminal variant; con, conw, sdl1, sdl1w, etc
...
QUERY_ITEM _THE_FAR query_item[] =
{
   ...
   {(CHARTYPE *)"fdisplay",        8, 8,ITEM_FDISPLAY,      1, 1,          LVL_FILE,QUERY_QUERY|             QUERY_EXTRACT|QUERY_MODIFY,extract_fdisplay               },
   ...
   {(CHARTYPE *)"select",          6, 3,ITEM_SELECT,        3, 3,          LVL_VIEW,QUERY_QUERY|QUERY_STATUS|QUERY_EXTRACT             ,extract_select                 },
   ...
   {(CHARTYPE *)"terminal",        8, 4,ITEM_TERMINAL,      3, 3,          LVL_FILE,QUERY_QUERY|QUERY_STATUS|QUERY_EXTRACT             ,extract_terminal               },
   ...
   {(CHARTYPE *)"utf",             3, 3,ITEM_UTF,           3, 3,          LVL_GLOB,QUERY_QUERY|QUERY_STATUS|QUERY_EXTRACT             ,extract_utf                    },
