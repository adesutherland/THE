...
/***********************************************************************/
{
   ...
   if ( (CURRENT_FILE->display_name = (CHARTYPE *)(*the_malloc)( 1 ) ) == NULL )
   {
      free_view_memory( TRUE, TRUE );
      display_error( 30, (CHARTYPE *)"", FALSE );
      TRACE_RETURN();
   ...
   strcpy( (DEFCHAR *)CURRENT_FILE->display_name, "" );
   /*
...
/***********************************************************************/
{
   ...
   if (CURRENT_FILE->display_name != NULL)
   {
      ...
      CURRENT_FILE->display_name = (CHARTYPE *)NULL;
   }
   /*
...
#ifdef USE_SDSLH
   sdslh_shutdown_file(CURRENT_FILE);
#endif
...
#ifdef USE_SDSLH
void sdslh_shutdown_file(FILE_DETAILS *fd) {
   CommunicationFunctions *comm = NULL;

   if (!fd)
      return;

   if (fd->cb) {
      cb_set_auto_relaunch(fd->cb, 0);
      cb_kill_parser_process(fd->cb);
      editor_wait_for_parser_threads();
      comm = fd->cb->communication_functions;
      fd->cb->communication_functions = NULL;
      free_code_buffer(fd->cb);
      fd->cb = NULL;
   }
   if (!comm)
      comm = fd->sdslh_comm;
...
void sdslh_init_file(FILE_DETAILS *fd) {
   ...
   if (!fd->colouring || !fd->parser || !fd->parser->is_sdslh_parser) {
      sdslh_shutdown_file(fd);
      return;
   }

   if (fd->cb && fd->sdslh_parser == fd->parser)
      return;
