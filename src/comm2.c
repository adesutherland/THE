...
DESCRIPTION
     ...
     EDITV GET, PUT, GETF, PUTF, GETSTEM and GETSTEMF are only valid from
     within a macro as they reference Rexx variables.  All other
     subcommands are valid from within a macro or from the command line.
     ...
     EDITV GETSTEM enables setting multiple Rexx variables from the
     equivalent EDITV variables based on the passed stem variable.
     The stem variable passed must contain a trailing period.

     EDITV GETF, PUTF, SETF, SETLF, SETFL, LISTF and GETSTEMF all work the same
...
#define EDITV_PARAMS  2
   ...
   else if ( equal( (CHARTYPE *)"getstem", word[0], 7 ) )
   {
      editv_type = EDITV_GETSTEM;
   }
   else if ( equal( (CHARTYPE *)"getstemf", word[0], 8 ) )
   {
      editv_type = EDITV_GETSTEM;
      editv_file = TRUE;
   }
...
#define GET_PARAMS  3
   ...
   build_screen(current_screen);
   display_screen(current_screen);
   INTENTIONALLY_UNUSED_VARIABLE(clip_type);
   TRACE_RETURN();
   return(RC_OK);
}
/*man-start*********************************************************************
